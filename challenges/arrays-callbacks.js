...
// store mapped scientific and animal names into displayNames array
...
// return zooAnimals array in specific format using .map method
// apply forEach method to lowCaseAnimalNames
const lowCaseAnimalNames = zooAnimals.map((item) => `${item.animal_name}`;);
// console.log, applying toLowerCase method to each resulting string
lowCaseAnimalNames.forEach(item => console.log(item.toLowerCase());
        ...
        */
        // use .filter to return obj based on condition of < 5 for the key population, array remains the same
        const lowPopulationAnimals = zooAnimals.filter(item => item.population < 5); console.log(lowPopulationAnimals);
        ...
        */
        // use .reduce with pop as the accumulator to store the total and start count at 0
         ...
         */
        // declare consume function with 3 parameters a, b, and cb
        function consume(a, b, cb) {
            // return the call to cb function
            return cb(a, b);
        }
        // when calling consume function cb can be any function
        consume("Ricky", "Bobby", greeting);
        ...
        // callBack function add parameters a and b
        add(a, b) {
            return a + b;
        }
        // callBack function multiply parameters a and b
        multiply(a, b) {
            return a * b;
        }
        // callBack function return parameters a and b
        greeting(a, b) {
